...
# create Quay secrets
oc create secret generic quay-database-credential --from-literal=database-username=quay --from-literal=database-password=$DBPASS --from-literal=database-root-password=$DBROOTPASS --from-literal=database-name=quay
...
oc create secret generic quay-config-app --from-literal=config-app-password=secret
oc create secret generic quay-redis-password --from-literal=password=$REDISPASS
oc create -f ../templates/quay-pull-secret.yaml
